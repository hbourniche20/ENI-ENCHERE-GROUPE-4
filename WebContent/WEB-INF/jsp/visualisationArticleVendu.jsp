						...
						<div class="col my-auto">
							<c:choose>
								...
								<c:otherwise>
									...
									<a href="${pageContext.request.contextPath }/utilisateur/profil?pseudo=${ article.getEncheres().get(article.getEncheres().size() - 1).getEncherisseur().getPseudo() }">${ article.getEncheres().get(article.getEncheres().size() - 1).getEncherisseur().getPseudo() }</a>	
								</c:otherwise>
							</c:choose>
						</div>
					</div>
